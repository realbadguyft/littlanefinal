...
const EVENT_DETAILS = {
    ...
    date: 'TBA',      // e.g. 'Aug 15, 2026'
    time: 'TBA',      // e.g. '20:00'
    venue: 'Pune',    // e.g. 'Symbiosis Grounds, Pune'
...
async function buildTicketPdf({ ticketId, name, email, gender, quantity, amount, createdAt }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        doc.font('Helvetica-Bold').fontSize(11).fillColor('#A855F7')
            .text(`${EVENT_DETAILS.stage}  ·  ${EVENT_DETAILS.admission}`, 24, y);
        y += 32;
        ...
        doc.font('Helvetica').fontSize(8).fillColor('#9a9a9a')
            .text(`Generated by ${EVENT_DETAILS.generatedBy} on ${new Date(createdAt).toLocaleString('en-IN')}`, 24, y);
        y += 30;
        ...
        // Littix wordmark — draw "Lit" + "tix" + purple dot manually centered
        const word1 = 'Lit', word2 = 'tix';
        doc.font('Helvetica-Bold').fontSize(14);
        const w1 = doc.widthOfString(word1), w2 = doc.widthOfString(word2), dotR = 3;
        const totalW = w1 + w2 + 10;
        let cx = (W - totalW) / 2;
        doc.fillColor('#0d0d0f').text(word1, cx, y, { continued: false, lineBreak: false });
        cx += w1;
        doc.fillColor('#A855F7').text(word2, cx, y, { continued: false, lineBreak: false });
        cx += w2 + 6;
        doc.circle(cx, y + 7, dotR).fill('#A855F7');
